...
func _resolve_hit(node: Node) -> void:
        ...
        if hurtbox.faction == faction:
                return
        # Check if the target is within the melee arc
        var target_node := hurtbox.get_parent()
        if target_node is Node2D:
                var to_target: Vector2 = (target_node as Node2D).global_position - global_position
                if to_target.length() > radius + 20.0:  # a bit of tolerance for body size
                        return
                var angle_to_target: float = to_target.angle()
                var angle_diff: float = abs(angle_difference(angle_to_target, _direction.angle()))
                if angle_diff > deg_to_rad(arc_degrees * 0.5):
